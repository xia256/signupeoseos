...
public:
    ...
    [[eosio::action]] void nop() {} // to produce a *.abi file
...
extern "C" void apply(uint64_t receiver, uint64_t code, uint64_t action)
{
    ...
    if (code != _self && action == name("transfer").value)
    {
        eosio::execute_action(
            eosio::name(receiver), eosio::name(code), &signupeoseos::transfer);
    }
    else if (code == _self && action == name("nop").value)
    {
        eosio::execute_action(
            eosio::name(receiver), eosio::name(code), &signupeoseos::nop);
    }
}
